...
class SequentialSearch(SearchBaseClass, ABC):
    ...
    # A* algorithm
    def astar(self, node_current):

        # OpenSet priorty Queue holding unvisited nodes.
        openSet = PriorityQueue()

        # Closed List holding visited nodes
        closed = []

        # Insert the initial Node to the set. Initial Cost (4.5)
        openSet.insert(node_current, self.cost_function(node_current))

        
        # Iterate through the set until the goal is reached.
        while not openSet.empty():
            
            # pop the node with the best score
            current = openSet.pop()

            # iterate, find and insert to the OpenSet all of its successors(neighbors)
            # also, examine if goal is reached, skip any nodes in touch of an obstacle

            
            for primitive_successor in current.get_successors():

                goal_flag = self.goal_reached(successor=primitive_successor, node_current=current)

                if goal_flag:
                    # print node path
                    print(f"Final path: {self.get_node_path(current)}")
                    return True


                collision_flag, child = self.take_step(successor=primitive_successor, node_current=current)

                #print(f"Child information:  {self.get_node_information(child)}")

                if collision_flag:          
                    continue

                f_score = self.evaluation_function(child)  

                openSet.insert(child, f_score)
                


            # after been visited and examined, save the node to a List 
            closed.append(current)

            if openSet.empty():
                print("Set is empty: " + str(openSet.empty()))

        print(f"Number of nodes visited: {len(closed)}")
    ...
    def execute_search(self, time_pause) -> Tuple[Union[None, List[List[State]]], Union[None, List[MotionPrimitive]], Any]:
        ...
        found_path = self.astar(node_current=node_initial)
