...
class SequentialSearch(SearchBaseClass, ABC):
    ...
    def heuristic_function2(self, node_current):
        ...
        distance = pow((goal[0] - node[0]), 2) + pow((goal[1] - node[1]), 2)
    ...
    def step_cost(self, node_current, node_successor):

        distance=self.cost_function(node_successor) - self.cost_function(node_current)

        return distance



    def idastar(self, node_current):


        # node_current is node_initial on the first call.
        threshold = self.heuristic_function(node_current) 

        while True:
            print("Iteration with threshold: " + str(threshold))
            distance = self.iterative_deepening_a_star(node_current, 0, threshold)

            if distance == float("inf"):
                # Node not found and no more nodes to visit.
                return False
            elif distance <0:
                # if we found the node, the fucntion returns the negative distance
    ...
    def iterative_deepening_a_star(self, current_node, distance, threshold):

        min = float("inf")


        # Check if latest node's successors are in proximity of the goal
        for primitive_successor in current_node.get_successors():

            goal_flag = self.goal_reached(successor=primitive_successor, node_current=current_node)
                
            if goal_flag:
                # We have found the goal node we we're searching for
                return -distance

        # Evalute the node cost

        #print("Visiting Node " + str(self.get_node_information(current_node)))
        estimate = distance + self.heuristic_function(current_node)
        #print(f"Estimate cost of {self.get_node_information(current_node)} node is: {estimate}")
        ...
        for primitive_successor in current_node.get_successors():
            ...
            collision_flag, child = self.take_step(successor=primitive_successor, node_current=current_node)
            ...
            t = self.iterative_deepening_a_star(current_node=child, distance=distance + self.step_cost(current_node, child), threshold=threshold)
            if t<0:
                #Node found
                return t 
            elif t < min:
    ...
    def execute_search(self, time_pause) -> Tuple[Union[None, List[List[State]]], Union[None, List[MotionPrimitive]], Any]:
        ...
        print("Obstacles: " + str(self.get_obstacles_information()))
        print("Goal: " + str(self.get_goal_information()))
        print("Start: " + str(self.get_node_information(node_initial)))
        """Enter your code here"""

        found_path = self.idastar(node_current=node_initial)
